...
${nuget_cmd} install src/IO.SprintPay/packages.config -o packages;
...
echo "[INFO] Run 'mcs' to build bin/IO.SprintPay.dll"
mcs -langversion:${langversion} -sdk:${sdk} -r:bin/Newtonsoft.Json.dll,bin/JsonSubTypes.dll,\
bin/RestSharp.dll,\
System.ComponentModel.DataAnnotations.dll,\
System.Runtime.Serialization.dll \
-target:library \
-out:bin/IO.SprintPay.dll \
-recurse:'src/IO.SprintPay/*.cs' \
-doc:bin/IO.SprintPay.xml \
-platform:anycpu
...
then
  ...
  echo "[INFO] bin/IO.SprintPay.dll was created successfully"
fi
